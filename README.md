...
2. Prepare directories on the host
   - 1. `$ sudo mkdir -p /opt/fs22/{docs,game,install,dlc}`
...
>[!WARNING]
If you already had an existing installation, delete the symlinked folder (`game`) from inside your install folder (`/opt/fs22/game`) before updating the game!  
